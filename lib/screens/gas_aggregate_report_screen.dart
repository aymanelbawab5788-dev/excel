...
/// بيانات سيارة واحدة متجمعة عبر كل شهور الفترة (غاز فقط)
class _GasCarAggregate {
  ...
  /// هل ظهرت كمية غاز فعلية أكبر من صفر في أي شهر خلال الفترة؟
  bool get hasAnyGasQuantity =>
      monthlyQuantities.values.any((q) => q > 0);
}

class GasAggregateReportScreen extends StatefulWidget {
  const GasAggregateReportScreen({super.key});

  @override
  State<GasAggregateReportScreen> createState() =>
      _GasAggregateReportScreenState();
}

class _GasAggregateReportScreenState
    extends State<GasAggregateReportScreen> {
  int? _fromMonth;
  int? _toMonth;
  ...
  final TextEditingController _fileNameController =
      TextEditingController(text: 'التقرير التجميعي غاز');

  // ===== ألوان التنسيق (نفس ألوان التقرير التجميعي الأصلي) =====
  ...
  Future<void> _generateReport() async {
    ...
    setState(() {
      _isProcessing = true;
      _preparedFile = null;
      _status = 'جاري تجميع تقرير الغاز...';
    });

    try {
      final output = _buildGasAggregateReport(_monthSlots);
  ...
  // =========================
  // قراءة وتجميع الملفات (غاز فقط)
  // =========================

  Uint8List _buildGasAggregateReport(List<_MonthSlot> slots) {
    final Map<String, _GasCarAggregate> carsByRecord = {};
    ...
    for (int slotIndex = 0; slotIndex < slots.length; slotIndex++) {
      ...
      // عمود "غاز" هو العمود الوحيد المطلوب من أعمدة الكميات.
      // إذا لم يكن موجودًا، تُعتبر كمية الغاز لهذا الشهر = 0.
      ...
      for (int r = headerRowIndex + 1; r < rows.length; r++) {
        ...
        // كمية الشهر = قيمة عمود "غاز" فقط (أو 0 لو العمود غير موجود).
        final quantity =
            gasIndex != null ? _numberValue(row, gasIndex) : 0;
        ...
        // كمية هذا الشهر (0 لو السيارة موجودة لكن بدون غاز).
    ...
    // السيارة تظهر في التقرير فقط إذا كان لديها كمية غاز فعلية
    // أكبر من صفر في شهر واحد على الأقل خلال الفترة.
    final filteredRecordOrder = recordOrder
        .where((rec) => carsByRecord[rec]!.hasAnyGasQuantity)
        .toList();
  ...
  Uint8List _writeOutput(
    ...
    Map<String, _GasCarAggregate> carsByRecord,
    List<String> recordOrder,
  ) {
    final outputExcel = ex.Excel.createExcel();

    final sheet = outputExcel['التقرير التجميعي غاز'];

    final defaultSheet = outputExcel.getDefaultSheet();

    if (defaultSheet != null &&
        defaultSheet != 'التقرير التجميعي غاز') {
    ...
    _writeCell(
      ...
      0,
      0,
      'التقرير التجميعي غاز - $periodLabel',
    ...
    if (encoded == null) {
      throw Exception(
        'فشل إنشاء ملف التقرير التجميعي غاز',
      );
    }
  ...
  Widget build(BuildContext context) {
    return Directionality(
      ...
      child: Scaffold(
        appBar: AppBar(
          title:
              const Text('التقرير التجميعي غاز'),
          centerTitle: true,
        ),
        body: Center(
